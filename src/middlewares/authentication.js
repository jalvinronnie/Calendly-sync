...
import { MONDAY_SIGNING_SECRET, MONDAY_OAUTH_CLIENT_SECRET} from '../constants/secret-keys.js';
...
export async function webhookAuthenticationMiddleware(req, res, next) {
  try {
      let { authorization } = req.headers;
      if (!authorization && req.query) {
          authorization = req.query.token;
